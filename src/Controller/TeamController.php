...
final class TeamController extends AbstractController
{
    ...
    public function positionEdit(
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $name = $this->nameFrom($form);
            // Renaming is scoped to the department the position already sits in — it does not
            // move, so the only question is whether that one department has the name free.
            if ($position->isLocked() || $this->team->nameIsFree($position->getDepartment(), $name, $position)) {
                $this->team->updatePosition($position, $name, $this->submittedPermissions($request));
                $this->addFlash('success', \sprintf('Updated “%s”.', TeamService::qualified($position)));

                return $this->redirectToRoute('app_team');
            }
            $this->addFlash('error', $this->clash($position->getDepartment(), $name));
        }
