 ...
 * real tabs ─ the module you are inside. It states where you are; it is never a catalogue.
 *
 * Two rules keep it honest:
 *  - MODULE-BLIND. No module slug is named here. The module under Modules is found by matching
 *    the `/areas/{uuid}/modules/{slug}` URL space against the area's own active AreaModule rows,
 *    and its link comes from the module provider's entry route ({@see ModuleEntryRouteResolver}).
...
final class SidebarExtension extends AbstractExtension
{
    ...
    {
        return [new TwigFunction('sidebar_nav', $this->build(...))];
    }
     ...
     * it; the module you are inside is the only thing that ever nests under Modules.
     *
    ...
    private function tabs(AreaOfInterest $area, bool $isCurrent, string $route, string $path): array
    {
        ...
        $modules = $isCurrent ? $this->currentModule($area, $path) : [];

        $rows = [
            ['Overview', 'dashboard_area_show', ['dashboard_area_show'], true, []],
            ['Modules', 'dashboard_area_modules_grid', ['dashboard_area_modules_grid', 'dashboard_area_modules'], $this->authorization->isGranted('module.view'), $modules],
            ['Zones', 'app_area_zones', ['app_area_zone'], true, []],
            ['Settings', 'dashboard_area_settings', ['dashboard_area_settings'], $this->authorization->isGranted('area.edit'), []],
        ];

        $tabs = [];
        foreach ($rows as [$label, $routeName, $lights, $granted, $tabModules]) {
    ...
    /**
     * The module the request is inside, as a single-entry list — module-blind: the slug comes off
     * the URL and only counts once the area really has that module switched on.
     *
     * @phpstan-return list<SidebarModule>
     */
    private function currentModule(AreaOfInterest $area, string $path): array
    {
        if (1 !== preg_match(self::MODULE_PATH, $path, $matches)) {
            return [];
        }
        ...
        foreach ($this->areaModules->activeForArea($area) as $areaModule) {
            ...
            $slug = $module?->getSlug();
            if (null === $module || $slug !== $matches['slug']) {
                continue;
            }
